...
struct Node {
  ...
  //overloads the < operator to compare the data of the two nodes
  ...
  //overloads == to check if the nodes' data is equal
   bool operator==(const Node& obj) {
       if(this->data == obj.data){
          return 1;
      }else{
 ...
 //overloads != to check if the nodes' data is not equal
   bool operator!=(const Node& obj) {
       if(this->data != obj.data){
          return 1;
      }else{
 ...
 public:
  ...
  void Sort(bool asc = true) {
    ...
    while (Start) {
      ...
      while (mint) {
        if (asc) {
          doSwap = (mint < minv); //used overloaded operator
        } else {
          doSwap = (mint > minv); //used the overloaded operator
        }
